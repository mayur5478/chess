$(function(){
	...
	var level = location.search.split('&&subChapter=')[0].split('&&chapter=')[0].split('&&level=')[1],
 						chapter =  location.search.split("&&subChapter=")[0].split("&&chapter=")[1],
 						subChapter = location.search.split('&&subChapter=')[1],
 						problem = location.search.split('&&subChapter=')[0].split('&&chapter=')[0].split('&&level=')[0].split('problem=')[1];
	if(level!=undefined){
		levelSelected = level.replace(/%20/g," ");
		chapterSelected = chapter.replace(/%20/g," ");
		subChapterSelected = subChapter.replace(/%20/g," ");
		var chapterPromise = addChapter(levelSelected);
		chapterPromise.done(function(){
			var subChapterPromise = addSubChapter(chapterSelected);
			subChapterPromise.done(function(){
				addProblem(subChapterSelected);
			});
		});
		  ...
		  $.ajax({url:'config/getProblems.php',data:{levelId:levelSelected,chapterId:chapterSelected,subChapterId:subChapterSelected}}).done(function(option){
		  		if(option!=""){
		  			...
		  			for(var index in problemList){
		  		 			var htmltext = "<div class='problem' id='"+problemList[index]+"'><a href='#' id='"+problemList[index]+"'>"+problemList[index]+"</a>&nbsp&nbsp&nbsp&nbsp<i class='fa fa-pencil fa-fw'></i>&nbsp<i class='fa fa-trash-o fa-fw'></i></div>";
	...
	that.onProblemClick = function(){
		  ...
		  buttons: {
		    "Ok": function() {
			    	...
			    	$( this ).dialog( "close" );
	  				/*&nbsp&nbsp&nbsp&nbsp<i class='fa fa-pencil fa-fw'></i>&nbsp<i class='fa fa-trash-o fa-fw'></i>*/
	  				var htmltext = "<div class='problem' id='"+newname+"'><a href='#' id='"+newname+"'>"+newname+"</a>&nbsp&nbsp&nbsp&nbsp<i class='fa fa-pencil fa-fw'></i>&nbsp<i class='fa fa-trash-o fa-fw'></i></div>";
